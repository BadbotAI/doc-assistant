  ...
  document: {
    ...
    capabilities: [
      ...
      { 
        ...
        query: '基于文档内容，回答我关于具体细节、背景信息、数据来源等方面的问���'
      },
    ],
  },
};
  ...
  files.forEach(file => {
    ...
    if (anns.length > 0) {
      const items = anns.map(a => {
        ...
        return `  ${icon} **${a.title}**：${a.comment.slice(0, 60)}...`;
      }).join('\n');
  ...
  if (mimeType.includes('pdf') || fileName.endsWith('.pdf')) {
    return {
      ...
      sections: [
        ...
        { type: 'heading', content: '二、核心内容' },
        { type: 'text', content: '本季度整体业绩呈上升趋势，各业务板块均实现正增长。核心业务增速放缓至12%，但拓展业务保持高速增��，同比增幅达到22.5%，成为新的增长引擎。' },
        { type: 'table', content: [
          ['指标', '本期', '上期', '变动'],
        ...
        { type: 'heading', content: '���、战略展望' },
        { type: 'text', content: '展望下一���段，建议加大对拓展业务的投入力度��同时完善风险管理体系。重点关注市场竞争态势变化和政策法���更新，确保业务可持续增长。' },
      ]
    };
  }
  if (mimeType.includes('word') || fileName.endsWith('.docx') || fileName.endsWith('.doc')) {
