  ...
  return (
    <div className="max-w-none">
      <div className="space-y-1.5">
        ...
        <div className="bg-slate-50/80 backdrop-blur-sm rounded-2xl rounded-tl-md px-5 py-4 border border-slate-100/80 max-w-md">
          <div className="flex items-center gap-2 mb-3.5">
            ...
            <span className={`text-[14px] font-medium ${parsingState.isComplete ? 'text-primary' : 'text-slate-700'}`}>
              {parsingState.isComplete ? '文档解析完成' : '正在解析您的文档...'}
            </span>
          </div>
                ...
                <div key={file.id} className="space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${colors.bg}`}>
                        <Icon className={`w-3 h-3 ${colors.icon}`} />
                      </div>
                      <span className="text-[13px] text-slate-600 truncate">{file.name}</span>
                    </div>
                    <span className={`text-[12px] font-medium flex-shrink-0 tabular-nums ${isDone ? 'text-primary' : 'text-stone-400'}`}>
                      {isDone ? <CheckCircle2 className="w-3.5 h-3.5 text-primary" /> : `${file.progress}%`}
                    </span>
                  </div>
                  <div className="h-1 bg-stone-200/60 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-700 ease-out ${isDone ? 'bg-primary' : colors.bar}`} style={{ width: `${file.progress}%` }} />
                  </div>
                </div>
              );
            })}
            ...
            <div className="flex items-center gap-1.5">
              {!parsingState.isComplete && (
                <div className="flex gap-0.5">
                  <div className="w-1 h-1 rounded-full bg-primary animate-[pulse_1.4s_ease-in-out_infinite]" />
                  <div className="w-1 h-1 rounded-full bg-primary animate-[pulse_1.4s_ease-in-out_0.2s_infinite]" />
                  <div className="w-1 h-1 rounded-full bg-primary animate-[pulse_1.4s_ease-in-out_0.4s_infinite]" />
                </div>
              )}
              <span className="text-[12px] text-slate-400 leading-none">{parsingState.stage}</span>
            </div>
          </div>
        </div>
      </div>
